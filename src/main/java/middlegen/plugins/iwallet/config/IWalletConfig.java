...
public class IWalletConfig {
    ...
    public static IWalletConfig getInstance() throws IWalletExceptionConfig {
        if (instance == null) {
            LogUtils.log("DAL配置没有初始化。");
            throw new IWalletExceptionConfig("DAL配置没有初始化。");
        }
     ...
     */
    public static synchronized void init(File configFile) throws IWalletExceptionConfig {
        if (instance != null) {
            return;
        }
        ...
        try {
            ...
            throw new IWalletExceptionConfig("Parse configuration file " + configFile.getPath() + " exception！", e);
        }
    }
