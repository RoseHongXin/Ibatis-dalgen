...
public final class FileProducer {
    ...
    public void setValidator(String validator) {
        if (StringUtils.isNotBlank(validator)) {
            this._validator = VALIDATOR_ROOT_PATH + StringUtils.capitalize(validator);
        }
    }
    ...
    public void generateForTables(VelocityEngine velocityEngine, Collection tableDecorators) throws MiddlegenException {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) instanceof IWalletTable) {
                String conf = ((IWalletTable) list.get(i)).getTableConfig().getConfidentiality();
                String integ = ((IWalletTable) list.get(i)).getTableConfig().getIntegrity();
